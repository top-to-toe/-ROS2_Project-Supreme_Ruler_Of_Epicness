...
class WaypointsLedNavigation : public rclcpp::Node
{
public:
    using FollowWaypoints = nav2_msgs::action::FollowWaypoints;
    using GoalHandleFollowWaypoints = rclcpp_action::ClientGoalHandle<FollowWaypoints>;

    WaypointsLedNavigation(const rclcpp::NodeOptions &options) : Node("waypoints_action_client", options)
    {
        using namespace std::placeholders;
        _action_client = rclcpp_action::create_client<FollowWaypoints>(this, "follow_waypoints");
        _timer = create_wall_timer(500ms, std::bind(&WaypointsLedNavigation::send_goal, this));
    }
    ...
    void send_goal()
    {
        ...
        send_goal_options.goal_response_callback = std::bind(&WaypointsLedNavigation::goal_response_callback, this, _1);
        send_goal_options.feedback_callback = std::bind(&WaypointsLedNavigation::feedback_callback, this, _1, _2);
        send_goal_options.result_callback = std::bind(&WaypointsLedNavigation::result_callback, this, _1);
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<WaypointsLedNavigation>(options);
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}